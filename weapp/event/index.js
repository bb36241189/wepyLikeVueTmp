...
//全局存储
var event = {}; //接收消息 传入params={name:"string 监听事件名",success:"function 监听事件回调",tg:"当前页面或App.js  传入this即可"}

var $on = function $on(params) {
  if (!params) {
    return false;
  }
  ...
  if (event[params.name]) {
    ...
    event[params.name] = [[params.tg, params.success]];
  }
...
var $remove = function $remove(params) {
  if (!params) {
    return false;
  }
  ...
  if (params.name && event[params.name]) {
    event[params.name] = event[params.name].filter(function (a) {
      return a[0] != params.tg;
    });
  } else {
    for (var key in event) {
      event[key] = event[key].filter(function (a) {
        return a[0] != params.tg;
      });
    }
  }
...
var pageStatus = function pageStatus(self) {
  if (self["onUnload"]) {
    var s = self["onUnload"];
